...
def nome_do_pokemon(numero):
    retorno = (requests.get(f'https://pokeapi.co/api/v2/pokemon/{numero}/')).json()
    return retorno['name']
...
def tipos_do_pokemon(nome):
    ...
    if retorno.status_code == 404:
        ...
        retorno = retorno.json()['types']
        lista = []
        tipos = []
        for i in retorno: lista.append(i['type']['name'])
        for i in lista: tipos.append(dic_tipos[i])
        return tipos
"""
...
def evolucao_anterior(nome):
    ...
    if retorno.status_code == 404:
        ...
        retorno = retorno.json()
        if retorno['evolves_from_species'] == None:
            return None
        else:
            return retorno['evolves_from_species']['name']
...
def evolucoes_proximas(nome):
    nome = nome.lower()
    retorno = requests.get(f'https://pokeapi.co/api/v2/pokemon-species/{nome}/')
    if retorno.status_code == 404:
        raise PokemonNaoExisteException
    else:
        retorno = retorno.json()
        url = retorno['evolution_chain']['url']
...
def nivel_do_pokemon(nome, experiencia):
    ...
    if retorno.status_code == 404:
        ...
        level_url = retorno.json()['growth_rate']['url']
        retorno = requests.get(level_url)
        retorno = retorno.json()['levels']
        for i in retorno:
